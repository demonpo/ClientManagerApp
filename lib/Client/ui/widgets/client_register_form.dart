...
class _ClientRegisterForm extends State<ClientRegisterForm> {
  final darkImagePicker = DarkImagePicker(
    size: 100,
  );
                ...
                child: MaterialButton(
                  ...
                  onPressed: () {
                    if (_fbKey.currentState.saveAndValidate()) {
                      var inscriptionDate = DateTime.parse(_fbKey
                          .currentState.value['inscriptionDate']
                          .toString());
                      var dealinePaymentDate =
                          inscriptionDate.add(Duration(days: 30));
                      print(_fbKey.currentState.value['nombres'].runtimeType);
                      print(_fbKey.currentState.value);
                      print(darkImagePicker.image == null
                          ? ""
                          : darkImagePicker.image.path);
                      clientBloc.addClient(Client(
                          photoPath: darkImagePicker.image == null
                              ? ""
                              : darkImagePicker.image.path,
                          ...
                          birthday:
                              _fbKey.currentState.value['birthday'].toString(),
                          inscriptionDate: _fbKey
                              .currentState.value['inscriptionDate']
                              .toString(),
                          deadLinePaymentDate: dealinePaymentDate.toString(),
                          debtValue: double.parse(
                              _fbKey.currentState.value['mensualidad']),
                          valorMensual: double.parse(
                              _fbKey.currentState.value['mensualidad'])));
                      Navigator.pop(context);
                    } else {
                      print(_fbKey.currentState.value['nombres'].runtimeType);
